defmodule AtlasWeb.Api.V1.SearchControllerTest do
  ...
  setup do
    ...
    on_exit(fn ->
      ["PHOTON_URL", "PLACEHOLDER_URL", "LIBPOSTAL_URL"] |> Enum.each(&System.delete_env/1)
    end)
  ...
  test "GET /api/v1/search returns 400 MISSING_PARAM with details.param when q missing", %{
    conn: conn
  } do
  ...
  test "GET /api/v1/search returns 502 UPSTREAM_ERROR when Photon returns 5xx", %{
    conn: conn,
    bypass: bypass
  } do
    Bypass.expect(bypass, fn c ->
  ...
  test "GET /api/v1/search returns 503 UPSTREAM_UNAVAILABLE when Photon is down", %{
    conn: conn,
    bypass: bypass
  } do
    Bypass.down(bypass)
