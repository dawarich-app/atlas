defmodule Atlas.Maps.RouteTest do
  ...
  test "plan returns trip + ok status", %{bypass: bypass} do
    ...
    assert %Result{features: %{trip: %{"summary" => _}}, upstream_status: "ok"} =
             Route.plan(from: %{lat: 52.5, lon: 13.4}, to: %{lat: 52.6, lon: 13.5}, mode: "auto")
  end

  test "plan returns unavailable when Valhalla down", %{bypass: bypass} do
    Bypass.down(bypass)
    assert %Result{upstream_status: "unavailable"} = Route.plan(from: %{lat: 0, lon: 0}, to: %{lat: 1, lon: 1}, mode: "auto")
  end
end
