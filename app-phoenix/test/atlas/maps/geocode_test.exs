defmodule Atlas.Maps.GeocodeTest do
  ...
  test "lookup with query returns {:ok, :forward, Result} on Photon success", %{bypass: bypass} do
    ...
    assert {:ok, :forward, %Result{features: features, upstream_status: "ok"}} =
             Geocode.lookup(query: "X")

    assert [%{name: "X"}] = features
  end

  test "lookup with lat+lon returns {:ok, :reverse, Result} on Photon reverse success", %{
    bypass: bypass
  } do
    Bypass.expect(bypass, fn conn ->
      case conn.request_path do
