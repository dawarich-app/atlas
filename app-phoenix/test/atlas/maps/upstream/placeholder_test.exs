  ...
  test "admin_for/2 returns admin map from /parser/search response", %{bypass: bypass, req: req} do
    Bypass.expect_once(bypass, "GET", "/parser/search", fn conn ->
      ...
      Plug.Conn.resp(
        conn,
        200,
        ~s([{"name":"Berlin","placetype":"locality","lineage":[{"country":{"name":"Germany"},"region":{"name":"Berlin"},"locality":{"name":"Berlin"}}]}])
      )
    end)

    assert %{country: "Germany", state: "Berlin", city: "Berlin"} =
             Placeholder.admin_for(req, text: "Berlin")
  end
